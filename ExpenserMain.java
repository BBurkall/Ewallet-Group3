...
public class ExpenserMain implements Expenser {
	...
	public void addMonthlyIncome(String source, double amount, String month) {
		userAtHand.addMonthlyIncome(source, amount, month);
	}
	 ...
	 * @param resultSet Base Wage ArrayList
	 * @param month Month as String
	 * @return New ArrayList of filtered data
	 * @throws SQLException 
	 */
	static ResultSet filterIncomesMonth(ResultSet resultSet, String month) throws SQLException {
		try {  
	...
	public int countIncomeEntries() {
		int incomeCount = 0;
		try {  
            User.stmt = User.conn.createStatement();
            
    		   ResultSet results = User.stmt.executeQuery("SELECT * from APP.Income");
       while(results.next())
        {
    	   incomeCount++;
            System.out.println("Source: "+ results.getString("source")+ ", Amount: "+ results.getString("amount")+ ", Month: " + results.getString("month"));
            	
        }
    		User.stmt.executeQuery("SELECT * FROM \"APP\".\"PERSON\""); }      
    	catch (SQLException sqlExcept){ sqlExcept.printStackTrace();}
		return incomeCount;
	}
	public void updateIncomeTable() throws SQLException {
		...
		for(int j = 0; j < countIncomeEntries(); j++ ) {
       ...
       while(results.next())
        {
			userAtHand.setBalance(userAtHand.getBalance());
			incomeRepPanel.incomeTable.setValueAt(results.getString("source"), i, 0);
			incomeRepPanel.incomeTable.setValueAt(results.getString("amount"), i, 1);
			incomeRepPanel.incomeTable.setValueAt(results.getString("month"), i, 2);
			i++;
        }      
		}
	 ...
	 * @throws SQLException 
	 */
	public void updateIncomeValues() throws SQLException {
	 ...
	 * @throws SQLException 
	 */
	public void updateExpenseValues() throws SQLException {
		userAtHand.setExpenses(getExpense(userAtHand.getSpending()));
		userAtHand.setBalance(getTotalIncome(userAtHand.getIncome()));
		updateMonthlySavings();
	 ...
	 * @param resultSet user's income ArrayList
	 * @return user's total income
	 */
	public double getTotalIncome(ResultSet resultSet) {
		double sum = 0.00f;
		try {  
            User.stmt = User.conn.createStatement();
